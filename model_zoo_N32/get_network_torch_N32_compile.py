...
sys.path.append('../newtorch')
from newtorch.curve_batch_compile import Curve
...
from model_zoo_N32.pdeNet_Ves_fat_factor_modein6_zerolevel_curvature import pdeNet_Ves_fat_factor_modein6_zerolevel_curvature
...
from model_zoo_N32.Net_ves_merge_advten_curv import Net_ves_merge_advten_curv
...
class MergedAdvNetwork:
    ...
    def preProcess(self, X):
        ...
        coords = torch.zeros((nv, 2*rep, 32), dtype=torch.float32, device=X.device)
        ...
        theta = torch.arange(N, device=X.device)/N*2*torch.pi
        theta = theta.reshape(N,1)
        bases = 1/N*torch.exp(1j*theta*torch.arange(N, device=X.device).reshape(1,N))
        # specify which mode
        rr, ii = torch.real(bases[:,s-1:t]), torch.imag(bases[:,s-1:t])
        basis = torch.concatenate((rr,ii), dim=0).T.reshape(1,rep,64)
        ...
        interleaved = stacked.reshape(nv, 2*rep, 2*N)
        
        return interleaved
    
    # @torch.compiler.disable(recursive=False)
    def forward(self, interleaved):
        ...
        with torch.inference_mode():
            ...
            Xpredict = self.model(interleaved.float()).reshape(-1,self.rep,2,64).transpose(0,1)
...
class TenSelfNetwork:
    ...
    def loadModel(self, model_path):
        ...
        # model = pdeNet_Ves_fat_factor_modein6_zerolevel_curvature(12, 1.5, 20)
        model.load_state_dict(torch.load(model_path, map_location=self.device, weights_only=True))
        # model = torch.compile(model, backend='cudagraphs')
        model.to(self.device)
        model.eval()
    ...
    def preProcess_curv(self, Xin):
        # Xin has shape (2N, nv)
        # XinitConv has shape (nv, 2, N)
        N = Xin.shape[0]//2
        nv = Xin.shape[1]
        c = Curve()
        _, _, curvature = c.diffProp(Xin)

        x_mean = self.input_param[0]
        x_std = self.input_param[1]
        y_mean = self.input_param[2]
        y_std = self.input_param[3]
        c_mean, c_std = 6.277393817901611, 9.243043899536133
        ...
        XinitShape_ = torch.zeros((nv, 3, 32), dtype=torch.float32).to(self.device)
        XinitShape_[:, 0, :] = (Xin[:N].T - x_mean) / x_std
        XinitShape_[:, 1, :] = (Xin[N:].T - y_mean) / y_std
        XinitShape_[:, 2, :] = (curvature.T - c_mean) / c_std
    ...
    def forward_curv(self, Xin):
        input = self.preProcess_curv(Xin.to(self.device))
        with torch.inference_mode():
            pred = self.model(input)
        tenPredstand = self.postProcess(pred)
        return tenPredstand
...
class TenSelfNetwork_curv:
    '''
    Input size (nv, 2, 32), 2 channels for x and y coords
    Output size (nv, 1, 32), 1 channel
    '''
    def __init__(self, input_param, out_param, model_path, device, oc):
        self.input_param = input_param # contains 4 numbers
        self.out_param = out_param # contains 2 numbers
        # self.model_path = model_path
        self.device = device
    ...
    def preProcess(self, Xin):
        ...
        _, _, curvature = self.oc.diffProp(Xin)

        x_mean, x_std, y_mean, y_std, c_mean, c_std = self.input_param
    ...
    def postProcess(self, pred):
        ...
        # N = pred.shape[2]
        # nv = pred.shape[0]
        out_mean, out_std = self.out_param
        pred = pred.squeeze(dim=1) # (nv, N)
...
#         model = Net_ves_merge_advten_curv(14, 2.4, 30, rep=31)
...
#         c = Curve()
#         _, _, curvature = c.diffProp(Xin)
#         Xin = torch.cat((Xin, curvature), dim=0)

#         # input = Xin[:, None].repeat_interleave(31, dim=-1).reshape(2, 32, nv, 31).to(self.device)
#         input = Xin[:, None].repeat_interleave(31, dim=-1).reshape(3, 32, nv, 31).to(self.device)
#         # use broadcasting
#         input[0] = (input[0] - self.input_param[:, 0])/self.input_param[:, 1]
#         input[1] = (input[1] - self.input_param[:, 2])/self.input_param[:, 3]
#         input[2] = (input[2] - self.input_param[:, 4])/self.input_param[:, 5] # curvature
        
#         return input.permute(2,3,0,1).reshape(nv, 3*31, -1)
...
class MergedTenAdvNetwork(torch.jit.ScriptModule):
    '''
    For each mode,
    Input size (nv, 2, 32), 2 channels for x and y coords
    Output size (nv, 2, 32), 2 channels for real and imag 
    ...
    def loadModel(self, model_path):
        ...
        # rep = t - s + 1 # number of repetitions
        # # prepare the network
        # model = Net_ves_merge_advten_curv(14, 2.4, 30, rep=rep)
        # dicts = []
        # # models = []
        # for l in range(s, t+1):
        #     model_path = f"/work/09452/alberto47/ls6/vesicle_advten/save_models/2025Feb_downsample32_ves_advten_mode{l}.pth"
        #     dicts.append(torch.load(model_path, map_location = self.device))
        ...
        #     new_weights[key] = torch.concat(tuple(params),dim=0)
        # model.load_state_dict(new_weights, strict=True)
        # torch.save(model.state_dict(),"../trained/2025Feb_merged_advten.pth")
    ...
    def postProcess(self, out):
        ...
        output = output.permute(3, 2, 0, 1) # shape: (31, nv, 2, 32)
        output[15] *= 0.0 # mode 17 target is zeors
        return output
...
class MergedNearFourierNetwork():
    ...
    def postProcess(self, out):
        ...
        reshaped_out[:, :, 16] *= 0.0 # mode 17 target is dropped
    ...
    #     model = self.model
    #     input_shape = (2*32, 32)
    #     batch_sizes = [2**i for i in range(1, 12)]  # Test batch sizes: 2, 4, 8, 16, ..., 2048
    #     results = {}
    ...
    #         try:
    #             inputs = torch.randn((batch_size, *input_shape), device=self.device)
    #             start.record()
    #             with torch.no_grad():
    #                 model(inputs)
    #             end.record()
    ...
    #         except RuntimeError:  # Handle out-of-memory (OOM) errors
    #             print(f"Batch {batch_size} is too large (OOM). Stopping.")
    #             break

    #     best_batch = max(results, key=results.get)
    #     print(f"\n Optimal Batch Size: {best_batch} ({results[best_batch]:.2f} samples/sec)")
    #     return best_batch
...
class MergedInnerNearFourierNetwork():
    ...
    def postProcess(self, out):
        ...
        reshaped_out =  torch.concat((out.permute(1, 0, 2, 3), # shape: (nv, 32, num_modes=31, 8)
                                torch.zeros(nv, 32, 1, 8, device=out.device)), # additional zeros shape: (nv, 32, 1, 8)
                                dim=-2) 
        reshaped_out[:, :, 16] *= 0.0 # mode 17 target is dropped
        # after postprocess, output velx_real, vely_real, velx_imag, vely_imag
        ...
        return torch.index_select(reshaped_out, dim=-1, index=torch.tensor([1,0], device=out.device)), \
                torch.index_select(reshaped_out, dim=-1, index=torch.tensor([3,2], device=out.device)), \
                torch.index_select(reshaped_out, dim=-1, index=torch.tensor([5,4], device=out.device)), \
                torch.index_select(reshaped_out, dim=-1, index=torch.tensor([7,6], device=out.device))
