...
class Starter_Block(nn.Module):
    def __init__(self, factor, ch, rep):
        ...
        self.conv1_1 = nn.Conv1d(3*rep, conv1_out_ch*rep, kernel_size=7, stride=2, padding=3, padding_mode='circular', groups=rep)
        self.conv1_2 = nn.Conv1d(3*rep, conv1_out_ch*rep, kernel_size=7, stride=2, padding=6, dilation=2, padding_mode='circular', groups=rep)
...
class Net_ves_merge_advten_curv(nn.Module):
    def __init__(self, num_blocks, factor, ch, rep):
        super(Net_ves_merge_advten_curv, self).__init__()
        self.factor = factor
        self.ch = ch
...
# net = Net_ves_merge_advten(4, 1.0, 20, 18)
# with torch.no_grad():
#     out1 = net(X.repeat_interleave(3, dim=1))

# with torch.no_grad():
#     out2 = net(X[:,:,None].expand(-1, -1, 3,  -1).reshape(3, 36, 12))
