...
class MLARM_manyfree_py:
    ...
    def time_step(self, Xold, tenOld):
        ...
        # fBend = vesicle.tracJump(Xold, np.zeros((N, nv)))
        # fTen = vesicle.tracJump(np.zeros((2 * N, nv)), tenOld)
    ...
    def translateVinfwTorch(self, Xold, vinf):
        ...
        in_param = self.advNetInputNorm
        out_param = self.advNetOutputNorm
        ...
        Xpredict = np.random.rand(len(mode_list), nv, 2, 256)
        ...
        for ij in range(len(mode_list) - 1):
            ...
            # Denormalize output
            real_mean, real_std, imag_mean, imag_std = out_param[imode - 1]
            for k in range(nv):
                pred[k, 0, :] = (pred[k, 0, :] * real_std) + real_mean
                pred[k, 1, :] = (pred[k, 1, :] * imag_std) + imag_mean
    ...
    def invTenMatOnVback(self, X, vinf):
        ...
        in_param = self.tenAdvNetInputNorm
        out_param = self.tenAdvNetOutputNorm
        ...
        for imode in modeList:
            if imode != 0:
                ...
                x_mean = in_param[imode-1, 0]
                x_std = in_param[imode-1, 1]
                y_mean = in_param[imode-1, 2]
                y_std = in_param[imode-1, 3]
                # for k in range(nv):
                #     input_net[k, 0, :] = (Xstand[:N, k] - x_mean) / x_std
                #     input_net[k, 1, :] = (Xstand[N:, k] - y_mean) / y_std
                input_net = np.concatenate(((Xstand[:N, None] - x_mean)/x_std, (Xstand[N:, None] - y_mean) / y_std), axis=1).T
                
                input_list.append(torch.tensor(input_net))

        # Xpredict = pyrunfile("tension_advect_predict.py", "output_list", input_shape=input_list, num_ves=int(nv))
        Xpredict = np.random.rand(len(modeList), nv, 2, 128)
        ...
        for ij in range(len(modeList) - 1):
            ...
            pred = np.array(Xpredict[ij], dtype=float)  # size(pred) = [1 2 256]

            # denormalize output
            real_mean = out_param[imode-1, 0]
            real_std = out_param[imode-1, 1]
            imag_mean = out_param[imode-1, 2]
            imag_std = out_param[imode-1, 3]
    ...
    def invTenMatOnSelfBend(self, X):
        ...
        for k in range(nv):
            # also destandardize
            tenPred[sortIdx[k], k] = (tenPredictStand[k] * out_std + out_mean) / scaling[k]**2
