...
class MLARM_manyfree_py(torch.jit.ScriptModule):
    ...
    def time_step_many_timing_noinfo(self, Xold, tenOld, nlayers=5):
        ...
        (
            ...
            info_rbf,
            info_stokes,
        ) = self.computeStokesInteractions_timing_noinfo(
            vesicle,
            vesicleUp,
            info_rbf,
            info_stokes,
        ...
        # use neural networks to calculate near-singular integrals
        start.record()
        farFieldtracJump, _, _ = self.computeStokesInteractions_timing_noinfo(
            vesicle,
            vesicleUp,
            info_rbf,
            info_stokes,
        ...
        torch.cuda.synchronize()
        print(
            f"relaxWTorchNet {start.elapsed_time(end)/1000} sec, containing standardization time."
        )
        ...
        torch.cuda.synchronize()
        print(
            f"x5 redistributeArcLength and alignCenterAngle {start.elapsed_time(end)/1000} sec."
        )
    ...
    def time_step_many_noinfo_exactVelLayer(self, Xold, tenOld, nlayers=4):
        ...
        dlayer = torch.linspace(
            0, 1 / N, nlayers, dtype=torch.float64, device=Xold.device
        )
        ...
        (
            ...
            info_rbf,
            info_stokes,
        ) = self.computeStokesInteractions_noinfo_exactLayers(
            vesicle,
            vesicleUp,
            info_rbf,
            info_stokes,
        ...
        # use neural networks to calculate near-singular integrals
        # start.record()
        farFieldtracJump, _, _ = self.computeStokesInteractions_noinfo_exactLayers(
            vesicle,
            vesicleUp,
            info_rbf,
            info_stokes,
    ...
    def computeStokesInteractions_timing_noinfo(
        self,
        vesicle,
        vesicleUp,
        info_rbf,
        ...
        # print(f'computeStokesInteractions EXACT {start.elapsed_time(end)/1000} sec, {torch.norm(far_field - far_field_1)/torch.norm(far_field)}')
        print(
            f"computeStokesInteractions noinfo EXACT {start_cuda.elapsed_time(end_cuda)/1000} sec"
        )
        ...
        torch.cuda.synchronize()
        print(
            f"x1 nearFieldCorrection SOLVE {start_cuda.elapsed_time(end_cuda)/1000} sec."
        )
    ...
    def computeStokesInteractions_noinfo_exactLayers(
        self,
        vesicle,
        vesicleUp,
        info_rbf,
        ...
        velx, vely, xlayers, ylayers = exactStokesSL_onlyself(
            vesicleUp_for_layers.X,
            vesicleUp_for_layers.sa,
            fup,
            Nup,
            Xup,
            vself,
    ...
    def computeStokesInteractions_timing(
        self,
        vesicle,
        vesicleUp,
        info_rbf,
            ...
            far_field_1 = torch.concat(
                (
                    fn(
                        ...
                        totalForceUp,
                        vesicle.X[:, : nv // 3],
                        info_stokes[0][info_stokes[2] < nv // 3],
                        info_stokes[1][info_stokes[2] < nv // 3],
                        info_stokes[2][info_stokes[2] < nv // 3],
                    ),
                    fn(
                        vesicleUp.X,
                        vesicleUp.sa,
                        totalForceUp,
                        vesicle.X[:, nv // 3 : 2 * nv // 3],
                        info_stokes[0][
                            (nv // 3 <= info_stokes[2]) & (info_stokes[2] < 2 * nv // 3)
                        ],
                        info_stokes[1][
                            (nv // 3 <= info_stokes[2]) & (info_stokes[2] < 2 * nv // 3)
                        ],
                    ...
                    fn(
                        ...
                        totalForceUp,
                        vesicle.X[:, 2 * nv // 3 :],
                        info_stokes[0][2 * nv // 3 <= info_stokes[2]],
                        info_stokes[1][2 * nv // 3 <= info_stokes[2]],
                        info_stokes[2][2 * nv // 3 <= info_stokes[2]] - 2 * nv // 3,
                        offset=2 * nv // 3,
                    ),
                ),
                dim=-1,
        ...
        start.record()
        self.nearFieldCorrectionUP_SOLVE_timing(
            vesicle, info_rbf, L, far_field_1, velx, vely, xlayers, ylayers
        )
        end.record()
        torch.cuda.synchronize()
        print(f"x1 nearFieldCorrection SOLVE {start.elapsed_time(end)/1000} sec.")
    ...
    def computeStokesInteractions(
        ...
        fn = allExactStokesSLTarget_broadcast
        if nv > 504:
            far_field = torch.concat(
                (
                    fn(
                        vesicleUp.X, vesicleUp.sa, totalForceUp, vesicle.X[:, : nv // 4]
                    ),
                    fn(
                        vesicleUp.X,
                        vesicleUp.sa,
        ...
        print(
            f"is close {torch.allclose(far_field, far_field_1)}, {torch.norm(far_field - far_field_1)/torch.norm(far_field)}"
        )
        ...
        self.nearFieldCorrectionUP_SOLVE(
            vesicle, info_rbf, L, far_field, velx, vely, xlayers, ylayers
        )
    ...
    def nearFieldCorrectionUP_SOLVE_timing(
        self, vesicle, info, L, far_field, velx, vely, xlayers, ylayers
    ):
        ...
        correction = torch.zeros(
            (N * nv, 2), dtype=far_field.dtype, device=far_field.device
        )
        ...
        torch.cuda.synchronize()
        print(
            f"---- nearFieldCorrection TRIANGULAR SOLVES {start.elapsed_time(end)/1000} sec."
        )
        ...
        correction = torch.sparse.mm(
            sp_matrix, coeffs.permute(1, 0, 2).reshape(nv * N * 5, 2)
        )
        end.record()
        torch.cuda.synchronize()
        t2 = start.elapsed_time(end) / 1000
